## Import individual broken UCL Player Birthplaces
importBrokenUCLLocs <- function(){
  uclPlayers$Birthplace[123] <<- "Buenos Aires, Argentina" # Javier Zanetti
  uclPlayers$Birthplace[230] <<- "Samtredia, Georgia" # Kakha Kaladze
  uclPlayers$Birthplace[288] <<- "Águilas, Murcia" # Jose Manuel Meca Garcia
  uclPlayers$Birthplace[302] <<- "Leytonstone, London, England" # David Beckham
  uclPlayers$Birthplace[329] <<- "Palma de Mallorca, Spain" # Raul Pareja
  uclPlayers$Birthplace[502] <<- "Vila Do Conde, Portugal" # Joao Festas
  uclPlayers$Birthplace[525] <<- "Joeuf, France" # Michel Platini
  uclPlayers$Birthplace[531] <<- "Bydgoszcz, Poland" # Zbigniew Boniek
  uclPlayers$Birthplace[569] <<- "Wolverhampton, England" # Gary Williams
  uclPlayers$Birthplace[577] <<- "Skegness, Lincolnshire, England" # Ray Clemence
  uclPlayers$Birthplace[581] <<- "Liverpool, England" # Jimmy Case
  uclPlayers$Birthplace[589] <<- "Clifton, Nottingham, England" # Viv Anderson
  ...
  uclPlayers$Birthplace[706] <<- "Cordoba, Spain" # Ramon Tejeda
